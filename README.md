...
Input files:
  --model arg                           Required argument: Path to model file. 
                                        Model has to be in PRISM format: e.g. 
                                        model.nm
  --property arg                        Required argument: Path to property 
                                        file. Property has to be of the form 
                                        e.g.: Pmax=? [F 'goal']

Configuration arguments (Can be specified via command line or config file):
...
Currently only reachability objectives are supported. Therefore, the program expects properties in either of the following forms: 
...
The set of states for which we want to check the reachability has to be labeled as 'goal'. This can be achieved by adding the following line at the bottom of the model file:
